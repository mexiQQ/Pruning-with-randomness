...
############################################################################################      
    ...
    ratios = [0.00005, 0.0001, 0.00008, 0.0003]
    ...
    for idx,s in enumerate(sparsities_schedule):
        ...
        for model_soup_id in range(model_soups_count):
            ...
            for i in range(len(optimizer.param_groups)):
                optimizer.param_groups[i]['lr'] = 0.0002
                optimizer.param_groups[i]['initial_lr'] = 0.0002

            ratio = ratios[model_soup_id % len(ratios)]
            pruner = get_pruner(args, model, s, ratio)
            args.max_train_steps = num_update_steps_per_epoch * 1 
            ...
            if idx != len(sparsities_schedule) - 1:
                metric = quickcheck(args, model, teacher, train_dataloader, accelerator, optimizer, lr_scheduler, val_dataloader, val_dataset, pruner, logger, model_soup_id, f"{s}_{ratio}")
            else:
                metric = quickcheck(args, model, teacher, train_dataloader, accelerator, optimizer, lr_scheduler, eval_dataloader, eval_dataset, pruner, logger, model_soup_id, f"{s}_{ratio}")
        ...
        mask = torch.load(f"{args.output_dir}/soup_{best_model_soup_id}/mask.pt")
        pruner = get_pruner(args, model, s, 0, mask)
        ...
        metric = train_one_phase(args, model, teacher, train_dataloader, accelerator, optimizer, lr_scheduler, eval_dataloader, eval_dataset, tokenizer, pruner, args.num_train_epochs, logger, -1)
...
def get_pruner(args, model, sparsity, sample_ratio, mask=None):
