...
############################################################################################      
    ...
    train_dataloader, eval_dataloader, eval_train_dataloader, eval_dataset, eval_train_dataset, train_dataset, num_labels, tokenizer, val_dataloader, val_dataset = get_dataset_and_dataloader(args)
    ...
    train_one_phase(args, model, teacher, train_dataloader, accelerator, optimizer, lr_scheduler, eval_dataloader, eval_dataset, tokenizer, pruner=None, epochs=1, logger=logger, model_soup_id=-1)
    ...
    # ratios = [0.00008, 0.0001, 0.0003, 0.0005]
    ratios = [args.sample_ratio]

    for idx,s in enumerate(sparsities_schedule):
        if s < 0.875:
            model_soups_count = args.sample_mask_count_1
        else:
            model_soups_count = args.sample_mask_count_2 

        best_model_soup_id = 0
        best_model_soup_metric = -1

        for model_soup_id in range(model_soups_count):
            logger.info("Load prevoius best model weigt")
            weight = torch.load(f"{args.output_dir}/pytorch_model.bin")
            optimizer_state = torch.load(f"{args.output_dir}/optimizer.pth")
            ...
            ratio = ratios[model_soup_id % len(ratios)]
            pruner = get_pruner(args, model, s, ratio)
            ...
            if idx != len(sparsities_schedule) - 1:
                metric = train_one_phase(args, model, teacher, train_dataloader, accelerator, optimizer, lr_scheduler, val_dataloader, val_dataset, tokenizer, pruner, args.num_train_epochs, logger, model_soup_id)
            else:
                metric = train_one_phase(args, model, teacher, train_dataloader, accelerator, optimizer, lr_scheduler, eval_dataloader, eval_dataset, tokenizer, pruner, args.num_train_epochs, logger, model_soup_id)
            
            if metric > best_model_soup_metric:
                best_model_soup_metric = metric
                best_model_soup_id = model_soup_id

        weight = torch.load(f"{args.output_dir}/soup_{best_model_soup_id}/pytorch_model.bin")
        optimizer_state = torch.load(f"{args.output_dir}/soup_{best_model_soup_id}/optimizer.pth")
        model.load_state_dict(weight)
        optimizer.load_state_dict(optimizer_state)
        
        if args.output_dir is not None:
            accelerator.wait_for_everyone()
            logger.info(f"**** Save model soup with best result: {best_model_soup_metric} ****")
            unwrapped_model = accelerator.unwrap_model(model)
            unwrapped_model.save_pretrained(args.output_dir, save_function=accelerator.save)
            torch.save(optimizer.state_dict(), f"{args.output_dir}/optimizer.pth")
            if accelerator.is_main_process:
        ...
        if idx != len(sparsities_schedule) - 1:
            ...
            logger.info(f"Val Dataset Result: {eval_metric}")
        else:
            eval_metric = evaluate_data(
                eval_dataloader,
                eval_dataset,
...
def get_dataset_and_dataloader(args):
    ...
    if task_name in ["mnli", "qnli", "qqp"]:
        train_dataset, val_dataset = train_test_split(train_dataset, test_size=len(eval_dataset)/len(train_dataset), shuffle=True, random_state=43)
    else:
        train_dataset, val_dataset = train_test_split(train_dataset, test_size=len(eval_dataset) * 20, shuffle=True, random_state=43)

    train_dataloader = DataLoader(
        train_dataset, shuffle=True, batch_size=args.per_device_train_batch_size
    )
    val_dataloader = DataLoader(val_dataset, batch_size=args.per_device_eval_batch_size)
    ...
    return train_dataloader, eval_dataloader, eval_train_dataloader, eval_dataset, eval_train_dataset,  train_dataset, num_labels, tokenizer, val_dataloader, val_dataset
...
def get_pruner(args, model, sparsity, sample_ratio):
